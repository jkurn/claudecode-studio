...
| 9    | L4→L5 | Vibe code the tool, not just the product.      | TBD — Aidan's pattern, one custom linter      |
...
4. **"What changes at each level" update.** The existing copy is good;
   merge in Aidan's three-loop framing so L5 = "three agents run while
...
- Every post should credit the speaker with a full name and, where
  possible, their company (Stephen Coleman / Stratum, Aidan Morgan /
...
- Tool recommendations (ast-grep, semgrep, Linear MCP) are Aidan /
